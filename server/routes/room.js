...
router.get('/name/:room_name', passport.authenticate('jwt', { session: false }), async (req, res) => {
    const room = await Room.findOne({ name: req.params.room_name })
        .populate('user', ['username', 'social', 'image', 'handle'])
        .populate('users.lookup', ['username', 'social', 'image', 'handle'])
        .exec();
...
router.post('/changeStatus', passport.authenticate('jwt', { session: false }), async (req, res) => {
    try {
        const room = await Room.findByIdAndUpdate(req.body.room_id, { status: "chatting", agentId: req.body.agent_id }, { upsert: true, new: true });
...
router.put('/setBackStatus/:agent_id', passport.authenticate('jwt', { session: false }), async (req, res) => {
    try {
        const room = await Room.updateMany({ agentId: req.params.agent_id }, { $set: { "status": 'rejoin' } });
